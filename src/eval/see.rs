...
impl SimpleScorer {
    ...
    pub fn eval_move_see(&self, board: &Board, mv: &Move) -> i32 {
        ...
        while !(attackers & board.color(attacker_color)).is_empty() {
            ...
            d += 1; 
            gain[d]  = self.mb.material_weights[mover].s() - gain[d-1]; // what you are taking less what opp has
...
mod tests {
    ...
    fn test_see() {
        ...
        let mv = b.parse_uci_move("a1a3").unwrap();
        assert_eq!(eval.eval_move_see(&b, &mv), 400);   // 900 - 500
        
        let b = Board::parse_fen("7k/8/8/8/1p6/q7/2N5/R6K w - - 0 1").unwrap();  //RN v qp
        let mv = b.parse_uci_move("a1a3").unwrap();
        assert_eq!(eval.eval_move_see(&b, &mv), 500);  // +q+p -R = 900 - 500 + 100  = 500
