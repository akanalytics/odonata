...
impl Configurable for MoveTimeEstimator {
    fn settings(&self, c: &mut Config) {
        ...
        c.set("mte.deterministic_clock", "type check default false");
    }
    ...
    fn configure(&mut self, c: &Config) {
        ...
        self.deterministic_clock = c.bool("mte.deterministic_clock").unwrap_or(self.deterministic_clock);
...
impl fmt::Display for MoveTimeEstimator {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln!(f, "deterministic clk: {}", self.deterministic_clock)?;
        Ok(())
    }
}
