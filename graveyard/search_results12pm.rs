...
impl fmt::Display for SearchResults {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln!(f, "seldepth {}", self.seldepth)?;
...
impl SearchResults {
    ...
    pub fn snapshot_bests(&mut self) {
        if self.score.is_numeric_or_mate() {
            // succesfully completed a part iter
            ...
            if self.multi_pv_index == 0 {
                ...
                if !self.board.is_legal_variation(&self.pv) {
                    error!("snapshot_bests: PV {} is invalid on board {}\n{:?}\n", self.pv, self.board, self.pv);
                    self.pv.truncate(1);
                }
    ...
    pub fn update_with_pv_change(&mut self, clock: &Clock, counts: &Counts, depth: Ply) {
        if self.score.is_numeric_or_mate() {
            self.mode = SearchResultsMode::PvChange;
        } else {
            self.mode = SearchResultsMode::NodeCounts;
        ...
        // check PV for validity
        if !self.board.is_legal_variation(&self.pv) {
            error!("update_with_pv_change: PV {} is invalid on board {}\n{:?}\n", self.pv, self.board, self.pv);
            self.pv.truncate(1);
        }
    }
