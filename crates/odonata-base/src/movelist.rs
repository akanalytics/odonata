    ...
    fn from(value: MultiVariation) -> Self {
        Self {
            moves: value
                .iter()
                .filter_map(|vs| {
                    // ignore empty variations
                    vs.var.first().map(|mv| (mv, vs.score))
                })
                .collect(),
        }
    }
...
impl ScoredMoveList {
    ...
    pub fn push(&mut self, mv: Move, sc: Score) {
    ...
    pub fn score_for_move(&self, mv: Move) -> Option<Score> {
        self.moves
            .iter()
            .filter_map(|(m, sc)| if *m == mv { Some(sc) } else { None })
            .nth(0)
            .cloned()
    }

    pub fn iter_moves(&self) -> impl Iterator<Item = Move> + '_ {
        self.moves.iter().map(|(mv, _sc)| mv).cloned()
    }
    ...
    pub fn validate(&self, b: &Board) -> anyhow::Result<()> {
        self.iter()
            .try_for_each(|(mv, _s)| mv.validate(b))
    ...
    pub fn to_uci(&self) -> String {
        // FIXME score uses to_pgn for to_uci to match parsing
        self.moves
            .iter()
            .map(|(mv, s)| format!("{mv}:{score}", mv = mv.to_uci(), score = s.to_pgn()))
            .join(" ")
            .to_string()
    }
...
mod tests_smv {
    ...
    fn test_scoredmovelist() {
        ...
        assert_eq!(moves.iter().next().unwrap().1.as_i16(), 34);
...
impl MoveList {
    ...
    pub fn parse_uci(s: &str, b: &Board) -> anyhow::Result<Self> {
        ...
        for mv in s.split_ascii_whitespace() {
            moves.push(Move::parse_uci(mv, b)?);
        }
        Ok(moves)
    }
    ...
    pub fn to_san(&self, b: &Board) -> String {
        let mut v = Vec::new();
        for &mv in self.iter() {
            debug_assert!(mv.is_valid(b), "mv {mv} is illegal for board {b}",);
            v.push(mv.to_san(b));
        }
        v.join(" ")
    }
...
impl From<MoveList> for Vec<Move> {
    fn from(value: MoveList) -> Self {
        value.iter().cloned().collect_vec()
    }
}
...
impl<const N: usize> From<[Move; N]> for MoveList {
    fn from(value: [Move; N]) -> Self {
        value.iter().cloned().collect()
    }
}
...
impl Board {
    ...
    pub fn to_san_variation(&self, var: &Variation) -> String {
        let mut s = String::new();
        let mut board = self.clone();
...
mod tests {
    ...
    fn test_movelist() -> Result<()> {
        ...
        let list = MoveList::parse_uci("1. a2a3, 2. b2b3  c2c4  ", &board)?;
        assert_eq!(list.to_string(), "a2a3, b2b3, c2c4");
        ...
        let s1: String = board
            .to_san_variation(&board.parse_san_variation(san)?)
            .split_whitespace()
            .collect();
