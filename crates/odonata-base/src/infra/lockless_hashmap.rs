...
impl<T: Debug + Default> Debug for AlignedVec<T> {
...
impl<T: Copy + Default> fmt::Display for UnsharedTable<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "array.len:{} array.cap:{} hitrate:{}% hits:{} misses:{} collisions:{} ",
            self.array.len(),
            self.array.capacity(),
...
mod tests {
    ...
    fn bench_unshared_table() {
        let mut prof_p = PerfProfiler::new("unshared_table_probe");
        let mut prof_s = PerfProfiler::new("unshared_table_store");
