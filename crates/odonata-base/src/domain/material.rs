    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        for &c in &Color::ALL {
            ...
            for &p in Piece::ALL.iter().rev() {
                write!(f, "{}", p.to_char(c).to_string().repeat(self.count(c, p) as usize))?;
            }
        }
        Ok(())
    }
...
impl Material {
    ...
    #[inline]
    pub fn centipawns_as_white(&self) -> i32 {
        Piece::ALL_BAR_KING
            .iter()
    ...
    pub const fn hash(&self) -> usize {
        let wq = self.count(Color::White, Piece::Queen);
        let wr = self.count(Color::White, Piece::Rook);
        let wb = self.count(Color::White, Piece::Bishop);
        let wn = self.count(Color::White, Piece::Knight);
        let wp = self.count(Color::White, Piece::Pawn);
        if wq > 1 || wr < 0 || wr > 2 || wb < 0 || wb > 2 || wn < 0 || wn > 2 || wp < 0 || wp > 8 {
            return 0;
        }
        let bq = self.count(Color::Black, Piece::Queen);
        let br = self.count(Color::Black, Piece::Rook);
        let bb = self.count(Color::Black, Piece::Bishop);
        let bn = self.count(Color::Black, Piece::Knight);
        let bp = self.count(Color::Black, Piece::Pawn);
...
mod tests {
    ...
    fn test_material() {
        ...
        assert_eq!(mat_Kkn.balance().to_string(), "n".to_string());
        assert_eq!(mat_full1.balance().to_string(), "".to_string()); // evenly matched
        assert_eq!(mat_some.balance().to_string(), "RNNqbpp".to_string());

        // centipawns
        #[allow(non_snake_case)]
        let mat_PPP = Material::from_piece_str("PPP").unwrap();
        let mat_p = Material::from_piece_str("p").unwrap();
        assert_eq!(mat_p.centipawns_as_white(), -100);
        assert_eq!(mat_PPP.centipawns_as_white(), 300);
        assert_eq!(mat_some.balance().centipawns_as_white(), -275); // R+N-Q = -75, N-b=-0, 2x-P=-200
        let board = Catalog::starting_board();
        assert_eq!(
            board.material().black().minors_and_majors().centipawns_as_white(),
            -3200
        );
        assert_eq!(board.material().white().minors_and_majors().centipawns_as_white(), 3200);
        assert_eq!(
    ...
    fn bench_material() {
        ...
        PerfProfiler::new("material-ctor").bench(
            || Material::from_board(&board), //
        );

        let m = Material::from_board(&board);
        PerfProfiler::new("material-total").bench(
            || m.total_count(), //
        );

        PerfProfiler::new("material-insuff").bench(
            || m.is_insufficient(), //
        );

        {
            let mut prof = PerfProfiler::new("material-manip");
            let mut prof2 = PerfProfiler::new("material-count");
            let board = Catalog::starting_board();
            let m = Material::from_board(&board);
            prof.start();
