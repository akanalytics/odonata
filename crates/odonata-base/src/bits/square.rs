...
impl FromStr for Square {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        // chars.next().unwrap().encode_utf8(&mut file);
        // chars.next().unwrap().encode_utf8(&mut rank);

        // let file =
        //     Bitboard::parse_file(std::str::from_utf8(&file)?)?;
        // let rank =
        //     Bitboard::parse_rank(std::str::from_utf8(&rank)?)?;
        // Ok((file & rank).square())
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::{globals::constants::*, infra::profiler::PerfProfiler};
    ...
    #[test]
    fn bench_square_uci() {
        let mut p = PerfProfiler::new("square_uci");
        p.bench(|| {
            for sq in Bitboard::all().squares() {
                black_box(&sq.uci());
            }
