...
use odonata_base::domain::node::{Counter, Event, Node, SearchType, Timing};
use odonata_base::domain::score::Score;
use odonata_base::domain::staticeval::{eval_with_outcome, StaticEval};
...
impl Search {
    pub fn alphabeta_root_search(
        ...
        *trail = Trail::new(pos.board().clone());
        ...
        let (score, category) = self.alphabeta("begin", trail, &mut pos.clone(), *n, Move::new_null())?;
        ...
        debug_assert!(
            self.current_variation.is_empty() || category == Event::UserCancelled || category == Event::SearchTimeUp
        );
    ...
    #[inline]
    fn alphabeta_static_eval(&mut self, pos: &Position, n: &Node) -> Score {
    ...
    pub fn alphabeta(
        ...
        if n.ply > 0 && (self.repetition.is_draw_by_repetition(pos) || pos.board().material().is_insufficient()) {
            ...
            return Ok((self.eval.eval_draw(pos.board(), n.ply).pov_score(), Event::NodeLeafDraw));
        ...
        // static eval
        let eval = self.alphabeta_static_eval(pos, &n);
        ...
        let mut sorted_moves = self.move_orderer.create_sorted_moves(n, pos.board(), tt_mv, last_move);
        if trail.path().len() < 2 {
            event!(target:"ab", Level::TRACE, "{var} generating moves...", var = trail.path(), );
        }
        let mut count = 0;
        let mut quiets = 0;
        while let Some((mt, mv)) = sorted_moves.next_move(pos.board(), self) {
            ...
            if bm.is_some() {
                if let Some(est) = self.can_futility_prune_move(mv, count, mt, before, pos.board(), eval, &n, ext) {
            ...
            let lmr = self.lmr(before, mv, count, quiets, mt, pos.board(), &n, bt, ext, tt_mv);
            ...
            if !(n.is_fw() && !self.pvs.permitted(bt, before, &n, count)) && lmr > 0 {
            ...
            if s == Score::INFINITY {
                match (lmr > 0, n.is_fw_equal_zw()) {
                    (..) if n.is_fw() && !self.pvs.permitted(bt, before, &n, count) => {
                    ...
                    (true, false) => {
                        ...
                        (s, ev) = self.alphabeta("zwrd", trail, pos, n.new_child().ext(ext - lmr).zw(), mv)?;
                        s = -s;
                        if s > n.alpha {
                            Metrics::incr_node(&n, Event::ReSearchZwFd);
                            (s, ev) = self.alphabeta("zwfd", trail, pos, n.new_child().ext(ext).zw(), mv)?;
                            s = -s;
                        }
                        if s > n.alpha {
                    ...
                    (true, true) => {
                        Metrics::incr_node(&n, Event::SearchZwRd);
                        (s, ev) = self.alphabeta("fwrd", trail, pos, n.new_child().ext(ext - lmr), mv)?;
                        s = -s;
                        if s > n.alpha {
                            Metrics::incr_node(&n, Event::ReSearchFwFd);
                            (s, ev) = self.alphabeta("fwfd", trail, pos, n.new_child().ext(ext), mv)?;
        ...
        if count == 0 {
            if n.is_qs() {
                ...
                let sc = eval_with_outcome(&self.eval, pos, &n);
...
mod tests {
    ...
    use odonata_base::domain::timecontrol::*;
    ...
    fn test_2_mates() -> Result<()> {
        ...
        for (i, epd) in positions.iter().enumerate() {
            ...
            println!("#{i} gives mate in {}", eng.search.score().mate_in().unwrap());
    ...
    fn test_mate_in_4() -> Result<()> {
        let mut eng = ThreadedSearch::new();
        eprintln!("{:?}", eng.search.eval);
        for epd in Catalog::mate_in_4().iter() {
            eng.start_game()?;
            let res = eng.search(epd.clone(), TimeControl::Depth(13)).unwrap();
            ...
            let final_board_pos = epd.board().make_moves(&res.pv());
            let current_board_score = eng.search.eval.static_eval_explain(&Position::from_epd(epd.clone()));
            let final_board_score = eng
                .search
                .eval
                .static_eval_explain(&Position::from_board(final_board_pos.clone()));
            if res.score().unwrap().mate_in() != Some(4) {
                eprintln!("result: {}\nexpected: {epd}", res.to_san(&epd.board()));
                eprintln!("b+pv = {final_board_pos}");
                eprintln!("score: {current_board_score}\nfscore: {final_board_score}");
                assert_eq!(res.score().unwrap().mate_in(), Some(4));
            };
        }
        Ok(())
    }
