...
impl Tree {
    ...
    fn children_of(&self, id: NodeId) -> Vec<&TreeNode> {
        ...
        // self.nodes.iter().filter(|tn| tn.parent == id).collect_vec()
        self.nodes.iter().filter(|tn| tn.parent == id).collect_vec()
    }
}
...
fn display_node(
    ...
    for (i, &child) in children.iter().enumerate() {
        ...
        if last && tree.children_of(child.id).is_empty() {
            for s in &indent {
                write!(f, "{}", if *s { "    " } else { "|   " })?;
            }
            writeln!(f)?;
...
impl Trail {
    ...
    pub fn push_move(&mut self, n: &Node, mv: Move) {
        ...
        debug_assert!(
            ...
            "push_move failed: ply:{ply} mv:{mv} on {b}\n{self}",
            b = self.board(ply)
        );
        let ply = ply as usize;
    ...
    pub fn record(&mut self, pos: Epd) {
