...
impl FeatureCategory {
    ...
    pub fn features(&self) -> impl Iterator<Item = Feature> + '_ {
        Feature::iter().filter(|f| f.category() == *self)
    }
...
impl Feature {
    ...
    pub const fn index_pst(base: Feature, p: Piece, sq: Square) -> Feature {
        let index = base.index() + p.index() * Square::len() + sq.index();
        Feature::from_index(index)
    }

    pub const fn material(p: Piece) -> Feature {
        match p {
            Piece::Pawn => Self::MaterialPawn,
            Piece::Knight => Self::MaterialKnight,
            Piece::Bishop => Self::MaterialBishop,
            Piece::Rook => Self::MaterialRook,
            Piece::Queen => Self::MaterialQueen,
            _ => unreachable!(),
        }
    }
    ...
    pub fn category_string(&self) -> &str {
        ...
        match *self {
            ...
            x if x >= ContemptPenalty && x <= WinBonus => "Winnable",
            _ => "Tempo",
        }
    }
    ...
    fn test_feature_basics() {
        ...
        for &p in &Piece::ALL {
            for sq in Square::all() {
                ...
                let _f = Feature::index_pst(Feature::PstP_A1, p, sq);
            }
        }
    }
}
