...
impl Algo {
    ...
    fn static_eval(&mut self, b: &Board, n: &Node) -> Score {
        ...
        if let Some(entry) = self.tt.probe_by_hash(b.hash()) {
            match self.tt.eval_from_tt {
                EvalFromTt::Never => b.static_eval(&self.eval),
    ...
    pub fn alphabeta(
        ...
        if n.is_null_move() {
            Metrics::incr_node(&n, Event::NodeNullMove);
        } else if n.is_razor() {
            Metrics::incr_node(&n, Event::NodeRazor);
        } else if n.is_zw() {
        ...
        while let Some((mt, mv)) = sorted_moves.next_move(b, self) {
            ...
            let lmr = self.lmr(b, mv, count, quiets, mt, &child_board, &n, bt, ext, tt_mv);
            ...
            if !(n.is_fw() && !self.pvs_permitted(bt, b, &n, count)) && lmr > 0 {
                if let Some(est) = self.can_futility_prune_move(
                    mv,
                    count,
                    mt,
            ...
            if s == Score::INFINITY {
                match (lmr > 0, n.is_fw_equal_zw()) {
                    (..) if n.is_fw() && !self.pvs_permitted(bt, b, &n, count) => {
                        Metrics::incr_node(&n, Event::SearchFwFd);
...
mod tests {
    ...
    fn test_mate_in_4() -> Result<()> {
        ...
        for (i, pos) in positions.iter().enumerate() {
            let mut search = Algo::new();
            let res = search.search(pos.clone(), TimeControl::Depth(10)).unwrap();
            // println!("{}", search);
            if pos.tags().pv.is_some() {
                let expected_pv = pos.pv()?;
                assert_eq!(
                    res.pv().to_string(),
