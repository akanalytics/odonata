use crate::{
    ...
    search::{
        ...
        razor::Razor, restrictions::Restrictions, timecontrol::TimeControl,
    },
...
impl Engine for Algo {
    ...
    fn options(&self) -> IndexMap<String, String> {
        ...
        let mb = format!("type spin default {} min 0 max 4000", self.tt.mb);
        let mpv = format!(
            "type spin default {} min 1 max 64",
            self.controller.multi_pv
        );
        map.insert("Clear Hash", "type button");
        map.insert("Hash", &mb);
        map.insert("MultiPV", &mpv);
        map.insert("Debug_Log_File", "string default \"\"");
        map.insert("UCI_AnalyseMode", "type check default false");
        map.insert("Analyse_Mode", "type check default false");
        map.insert("UCI_Opponent", "type string default \"\"");
        // map.insert("nodestime", "");
        map.insert("OwnBook", "type check default false");
        map.insert("Book File", "type string default \"\"");
        map.insert("Best Book Move", "type check default true");
    ...
    fn set_option(&mut self, name: &str, value: &str) -> anyhow::Result<()> {
        match name {
            ...
            "Book File" => {
                self.opening_book.book_file = value.parse()?;
                self.opening_book.reload()?;
            }
...
mod tests {
    ...
    fn test_black_opening() {
        ...
        assert_eq!(sr.unwrap().supplied_move().unwrap().to_uci(), "b8c6");
    }
    ...
    fn test_algo_as_engine() {
        ...
        assert_eq!(sr.unwrap().supplied_move().unwrap().to_uci(), "b1c3");
    }
