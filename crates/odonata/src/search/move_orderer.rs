use crate::{
    ...
    mv::Move,
    other::Phase,
    piece::{Color, MoveType, Piece, Ply},
    search::{algo::Algo, stack::Stack},
...
impl MoveOrderer {
    ...
    pub fn quiet_score(
        ...
        if self.discovered_check_sort_bonus != 0.0  && b.make_move(mv).is_in_check(c.opposite()) {
            score += self.discovered_check_sort_bonus;
        }
    ...
    fn calc_next_move_(&mut self, b: &Board, algo: &mut Algo) -> Option<(MoveType, Move)> {
        ...
        if self.index < self.moves.len() {
            ...
            if move_type == MoveType::GoodCaptureUpfrontSorted || move_type == MoveType::GoodCapture
            {
                ...
                let see_cutoff = i32::from(see_cutoff.as_i16());
...
mod tests {
    ...
    use crate::{
        ...
        movelist::*,
        piece::*,
        search::{algo::*, engine::*, timecontrol::TimeControl},
    };
    ...
    fn test_prior_pv() {
        ...
        pv.extend_from_slice(&vec![a1a2, a1a3, a1a4]);
        ...
        let mut moves = moves_orig.clone();
        variation.extend_from_slice(&vec![a1a2, c1c2]);
        Algo::order_from_prior_pv(&mut moves, &variation, &pv);
        assert_eq!(moves_orig.to_string(), "b1a2, b1a3, b1a4, a1a3, a1a4, a1a2");
        ...
        let mut variation = Variation::new();
        variation.extend_from_slice(&vec![a1a2, a1a3, c1c2]);
        let mut moves = moves_orig.clone();
        Algo::order_from_prior_pv(&mut moves, &variation, &pv);
        assert_eq!(moves.to_string(), "b1a2, b1a3, b1a4, a1a3, a1a4, a1a2");
        ...
        let mut variation = Variation::new();
        variation.extend_from_slice(&vec![a1a2]);
        let mut moves = moves_orig.clone();
        Algo::order_from_prior_pv(&mut moves, &variation, &pv);
        ...
        let mut moves = moves_orig.clone();
        let mut variation = Variation::new();
        variation.extend_from_slice(&vec![a1a2, a1a3]);
        Algo::order_from_prior_pv(&mut moves, &variation, &pv);
