use crate::{
    bits::Square,
    boards::Board,
    cache::tt2::{TranspositionTable2, EvalFromTt},
    clock::Clock,
...
impl RunQs<'_> {
    ...
    pub fn qsearch(&mut self, n: &Node, bd: &mut Board, lm: Option<Move>) -> Result<Score, Score> {
        ...
        if cfg!(debug_assertions) && false {
            ...
            pos.tags_mut().pv = Some(self.trail.pv(n).clone());
            pos.tags_mut().sv = Some(self.trail.path().clone());
            pos.tags_mut().acn = Some(self.clock.q_nodes() - q_nodes);

            match res {
                Ok(score) => {
                    pos.tags_mut().comments[0] = Some("Pv".into());
                    pos.tags_mut().centipawn_evaluation = Some(score.as_i16() as i32);
                }
                Err(score) => {
                    pos.tags_mut().comments[0] = Some("Cut".into());
                    pos.tags_mut().centipawn_evaluation = Some(score.as_i16() as i32);
                }
            };
...
mod tests {
    ...
    fn test_qs_catalog() -> Result<()> {
        ...
        for pos in catalog {
            ...
            eng.algo.qs.probe_tt = false; 
            eng.search_sync();
            // debug!("{}", search);
            println!(
                "search:  {pv_act:<20}  {pos_act}\nexpected:{pv_exp:<20}  {pos_exp}\n",
                pv_act = eng.algo.pv().to_string(),
                pos_act = eng.algo.results_as_position(),
                pv_exp = pos.pv()?.to_string(),
                pos_exp = pos,
