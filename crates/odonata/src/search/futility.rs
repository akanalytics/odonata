use crate::{
    boards::Board,
    eval::{eval::Eval, score::Score, weight::Weight},
...
use crate::piece::{MoveType, Piece, Ply};
...
impl Algo {
    ...
    pub fn can_futility_prune_move(
        ...
        if !self.futility.prune_extensions && ext != 0 {
            Metrics::incr_node(n, Event::FutilityDeclineExt);
            return None;
        }
    ...
    fn eval_move_material(&self, before: &Board, eval: &Eval, mv: Move) -> Score {
        if self.futility.standard_piece_evals {
            let mut cp = 0;
            ...
            Score::from_cp(wt.interpolate(before.phase(&eval.phaser)) as i32)
        }
    }
