...
use crate::{
    ...
    other::{outcome::Outcome, Phase},
    Bitboard, Color,
};

use super::{
    eval::{FeatureOld, WeightsVector},
    weight::Weight,
};

pub trait ScorerBase {
    fn accumulate(&mut self, i: FeatureOld, w_value: i32, b_value: i32);
    fn accum(&mut self, c: Color, i: FeatureOld, value: i32);
    fn set_bits(&mut self, i: FeatureOld, bits: Bitboard);
...
impl<'a> ScorerBase for TotalScore<'a> {
    ...
    #[inline]
    fn set_bits(&mut self, _i: FeatureOld, _bits: Bitboard) {}
...
pub struct ExplainScore {
    ...
    vec:              Vec<(FeatureOld, i32, i32, i32)>, // w-val, b-val, feature-index
    weights:          Option<WeightsVector>,
    bitboards:        HashMap<FeatureOld, Bitboard>,
}
...
impl ExplainScore {
    ...
    // (index, net feature)
    pub fn values(&self) -> impl Iterator<Item = (FeatureOld, usize, i32)> + '_ {
        self.vec.iter().map(|e| (e.0, e.3 as usize, (e.1 - e.2)))
    }

    pub fn value(&self, i: FeatureOld) -> i32 {
        self.vec
            .iter()
            .find(|&e| i == e.0)
            .map(|e| (e.1 - e.2))
    ...
    pub fn score_for_category(&self, s: &str) -> Weight {
        match self.weights {
            None => Weight::zero(),
            Some(ref wv) => self
                .vec
    ...
    pub fn write_csv<'a, W: Write>(
        ...
        writeln!(
            ...
            FeatureOld::all().iter().map(|f| f.name()).join(",")
        )?;
        let mut count = 0;
        for r in iter {
            count += 1;
            for c in &FeatureOld::all() {
                write!(f, "{},", r.value(*c))?;
            }
            writeln!(
                f,
...
mod tests {
    use super::*;
    use crate::{
        catalog::Catalog, eval::calc::Calc, other::Phaser, search::engine::ThreadedSearch,
        test_log::test,
    };
