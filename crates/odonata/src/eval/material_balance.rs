use crate::{
    ...
    domain::Material,
    eval::weight::Weight,
    infra::{component::Component, utils::file_open},
    mv::Move,
    piece::{Color, Piece},
};
...
impl MaterialBalance {
    ...
    fn derived_store(&self, mat: &Material, value: f32) {
        let v_pos = value.to_bits();
        let v_neg = (-value).to_bits();
    ...
    // inerior mutability
    fn init(&self) {
        // "zero" existing ones to the NICHE value indicating "not set"
    ...
    fn parse_and_store(&self, k: &str, v: &str) -> Result<()> {
        ...
        let cp = v
            .trim()
            .parse::<f32>()
            .map_err(|err| anyhow!("{} - unable to parse number '{}'", err, v))?;
    ...
    fn init_from_raw_stats(&self) {
        info!(
            "creating derived material balance table from {} raw internal statistics",
            RAW_STATS.len()
        );
    ...
    // fn log_inconsistency(text: &str, mat: &Material, score: i32, other: &Material, other_score: i32)  {
    //     trace!("{:>20} {:>32}={:>5} > {:>30}={:>5} ----> [{}] = {} ({})", mat.to_string(), cp, other, other_cp, other, new_cp, new_cp - cp );
    // }
}
