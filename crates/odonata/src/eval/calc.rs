use crate::bits::{bitboard::Dir, precalc::Pawns, CastlingRights, PreCalc, Square};
// use crate::board::analysis::Analysis;
use crate::{
    boards::Board,
    eg::{EndGame, LikelyOutcome},
    eval::eval::FeatureOld,
...
use super::{eval::Attr, scorer::ScorerBase};
use Attr::*;
    ...
    fn endgame(&mut self, scorer: &mut impl ScorerBase, b: &Board) -> bool {
        ...
        if let Some(winner) = endgame.try_winner(b) {
            if let Some((metric1, metric2)) = endgame.metrics(winner, b) {
                ...
                scorer.accum(winner, CertainWinBonus.as_feature(), 1);
                return true;
            }
...
// #[inline]
// fn _pawns(&mut self, c: Color, s: &mut impl ScorerBase, b: &Board) {
//     let us = b.color(c);
//     let them = b.color(c.opposite());
