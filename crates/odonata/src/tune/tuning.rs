...
use crate::{
    ...
    infra::{
        component::Component,
        metric::Metrics,
        utils::{sigmoid, win_probability_from_cp_and_k},
    },
    other::{outcome::Outcome},
    position::Position,
    search::{engine::ThreadedSearch, node::Timing},
    Color,
};
use anyhow::Result;
...
impl Tuning {
    ...
    pub fn feature_names(&self) -> Vec<String> {
        let s = FeatureOld::all().into_iter().map(|f| f.name() + ".s");
        let e = FeatureOld::all().into_iter().map(|f| f.name() + ".e");
        s.interleave(e).collect_vec()
    }

    /// [index, feature, weight] for a single fen
    pub fn sparse_feature_vec(&self, idx: usize) -> Vec<(usize, FeatureOld, f32)> {
        ...
        eng.tuner.explains = positions
            ...
            .filter_map(|(_i, pos)| {
                ...
                if eng.tuner.ignore_likely_endgames
                    && explain.value(FeatureOld::Discrete(Attr::WinBonus)) != 0
                {
    ...
    pub fn calc_player_win_prob_from_pos(&self, pos: &Position) -> (f32, String) {
        if let Some(s) = &pos.tags().comments[9] {
...
mod tests {
    ...
    use super::*;
    use crate::{
        eval::{eval::Attr, weight::Weight},
        infra::{
            profiler::PerfProfiler,
            utils::{read_file, Formatting},
        },
    };
    use anyhow::Context;
    use test_log::test;
    ...
    fn test_tuning_load() {
        ...
        let file = "../../epd/tuning/quiet-labeled-small.epd";
    ...
    fn test_tuning_mse() {
        ...
        let file = "../../epd/quiet-labeled-small.epd";
        let _count = Tuning::upload_positions(
            &mut eng,
            Position::parse_many_epd(read_file(file).unwrap()).unwrap(),
        )
