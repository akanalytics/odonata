...
use crate::{
    bits::castling::CastlingRights,
    infra::utils::{StringUtils, Uci},
    movelist::MoveList,
    piece::{Color, Ply},
};
...
impl TryFrom<HashMap<String, String>> for Position {
    ...
    fn try_from(map: HashMap<String, String>) -> Result<Self> {
        ...
        for (k, v) in map.iter() {
            ...
            let tags = Tags::parse_single_tag(p.board(), &(k.to_string() + " " + &v))
                .with_context(|| format!("in tag '{}' with value '{}'", k, v))?;
            p.tags_mut().fill_from(tags);
        }
        Ok(p)
    }
}
...
impl Into<HashMap<String, String>> for Position {
    fn into(self) -> HashMap<String, String> {
        // FIXME clones!
        let mut map = self.tags.clone().to_hash_map(&self.board());
...
impl Serialize for Position {
        ...
        S: Serializer,
    {
        ...
        // FIXME clones!
        let map = self.tags.clone().to_hash_map(self.board());
...
impl Position {
    ...
    pub fn parse_epd(epd: &str) -> Result<Self> {
        ...
        if words[3] == "-" {
            ...
            pos.board.set_en_passant(Some(Square::parse(words[3])?));
        };
        ...
        if words.len() >= 6 {
            ...
            if let Ok(hmvc) = hmvc {
                if let Ok(fmvn) = fmvn {
                    pos.board.set_halfmove_clock(hmvc);
                    pos.board.set_fullmove_number(fmvn);
                    remaining = StringUtils::trim_first_n_words(epd, 6);
                }
            }
        }
        pos.tags = Tags::parse_epd_tags(&pos.board, remaining)?;
    ...
    pub fn parse_many_epd(iter: impl IntoIterator<Item=impl AsRef<str>>) -> Result<Vec<Position>> {
    ...
    where
        I: IndexedParallelIterator<Item = String>,
    {
        ...
        par_iter
            ...
            .filter_map(|(line_num, line)| {
                ...
                if s.is_empty() || s.trim_start().starts_with('#') {
                    ...
                    Some(
                        Position::parse_epd(&s)
                            .with_context(|| format!("parsing EPD '{s}' at line {line_num}")),
                    )
                }
            })
...
impl Position {
    ...
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        if let Some(sv) = &self.tags.sv {
            sv.validate(self.board())?;
        }
        self.tags.validate(&self.board_after())?;
        Ok(())
    }
    ...
    pub fn board_after(&self) -> Board {
        self.board().make_moves_old(self.supplied_variation())
    }
    ...
    pub fn set(&mut self, tag: Tags) {
        self.tags.fill_from(tag);
    }
    ...
    pub fn branching_factor(&self) -> f64 {
        match self.tags.branching_factor {
            Some(v) => v,
            _ => 0.0,
        }
    }

    pub fn bm(&self) -> &MoveList {
        match &self.tags.best_moves {
            Some(v) => v.deref(),
            _ => panic!("Missing bm in position {}", self),
        }
    }

    pub fn sm(&self) -> Result<Move> {
        if let Some(mv) = self.tags.supplied_move {
            Ok(mv)
        } else {
    ...
    pub fn sq(&self) -> Bitboard {
        match self.tags.squares {
            Some(v) => v,
            _ => panic!("Missing sq in position {}", self),
        }
    }

    pub fn ce(&self) -> i32 {
        match self.tags.centipawn_evaluation {
            Some(v) => v,
            _ => panic!("Missing ce in position {}", self),
        }
    }

    // acd analysis count depth [3]
    pub fn acd(&self) -> Result<Ply> {
        if let Some(acd) = self.tags.acd {
            Ok(acd)
        } else {
    ...
    pub fn acn(&self) -> u64 {
        match self.tags.acn {
            Some(v) => v,
            _ => panic!("Missing acn in position {}", self),
        }
    }

    pub fn dm(&self) -> u32 {
        match self.tags.direct_mate {
            Some(v) => v,
            _ => panic!("Missing dm in position {}", self),
        }
    }
    ...
    pub fn id(&self) -> &str {
        match &self.tags.id {
            Some(v) => v,
            _ => panic!("Missing id in position {}", self),
        }
    }
...
mod tests {
    ...
    fn test_epd_parse_many() -> Result<()> {
        ...
        assert_eq!(
            epds[0].tags.comments[0],
            Some("Henry Buckle vs NN, London, 1840".to_string())
        );
    ...
    fn test_position_uci() -> Result<()> {
        ...
        let bd2 = pos.board.make_move("a2a3".mv(&pos.board()));
    ...
    fn test_pos_basics() -> Result<()> {
        ...
        *pos.board_mut() = Board::parse_fen(Catalog::STARTING_POSITION_FEN).unwrap();
        ...
        assert_eq!(pos.bm().to_string(), "e2e4");

        let mut pos = Position {
            board: Catalog::starting_board(),
            tags:  Tags::default(),
        };
        pos.tags.best_moves = Some(Box::new("e4, c4, a4".moves(&pos.board)));
        pos.tags.pv = Some("e4 e5 d3".var(&pos.board()));
        assert_eq!(pos.bm().to_string(), "e2e4, c2c4, a2a4");
    ...
    fn test_pos_custom() -> Result<()> {
        let mut pos = Position::default();
        *pos.board_mut() = Board::parse_fen(Catalog::STARTING_POSITION_FEN).unwrap();
        pos.tags_mut().squares = Some("e4 e5 e6".squares());
        assert_eq!(pos.sq(), e4 | e5 | e6);

        pos.tags_mut().squares = Some(Bitboard::empty());
        assert_eq!(pos.sq(), Bitboard::empty());
        Ok(())
    }
    ...
    fn test_parse_epd_file() -> Result<()> {
        ...
        let positions = Position::parse_many_epd(read_file("../../epd/com15.epd")?)?;
        for p in positions {
            println!(">> {}", p);
        }
        Ok(())
