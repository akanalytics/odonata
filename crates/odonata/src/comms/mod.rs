...
use crate::{
    comms::bench::Bench, infra::utils::ToStringOr, search::timecontrol::TimeControl,
...
pub fn main() -> anyhow::Result<()> {
    ...
    let mut uci = UciServer::new(settings);
    uci.strict_error_handling = cli.strict;

    match cli.command.unwrap_or(Cmd::Engine) {
        Cmd::ShowConfig => uci.add_prelude("uci; show_config; quit"),
        Cmd::Bench => uci.add_prelude("position startpos; go depth 11; isready; bench; quit"),
        Cmd::Profile => {
            Bench::profile_me();
            return Ok(());
        }
        Cmd::Perft { depth } => uci.add_prelude(&format!("perft {depth}; quit")),
        Cmd::PerftCat { depth } => uci.add_prelude(&format!("perft_cat {depth}; quit")),

        Cmd::Search { time_control } => {
            Bench::search(time_control, cli.threads);
            return Ok(());
        }

        Cmd::Uci { command } => uci.add_prelude(&(command + "; isready; quit")),
        Cmd::Engine => uci.add_prelude("compiler"),
    };
    uci.run();
    Ok(())
}
