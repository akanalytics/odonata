use crate::{
    boards::Board,
    catalog::{Catalog, CatalogSuite},
    infra::{
        utils::read_file,
        version::{built_info, Version},
    },
    position::Position,
...
impl Rpc for RpcImpl {
    ...
    fn eval(&self, board: Board) -> Result<Position> {
        let res = board.outcome().as_pgn();
        let mut p = Position::from_board(board);
        p.tags_mut().result = Some(res);
        Ok(p)
    }
...
mod tests {
    ...
    fn test_json_rpc() {
        ...
        assert_eq!(rpc.invoke(request1), Some(response));
    }

    #[test]
    fn position_download_test() -> anyhow::Result<()> {
        let rpc = RpcImpl::new(Arc::new(Mutex::new(ThreadedSearch::new())));
        rpc.position_upload("../../epd/tuning/quiet-labeled-small.epd".to_string())?;
