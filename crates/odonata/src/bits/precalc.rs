...
mod tests {
    ...
    fn test_precalc_pawns() {
        ...
        let calced = PreCalc::instance().pawn_front_span_union_attack_span(Color::White, Square::B2);
        let expect = (Bitboard::FILE_A | Bitboard::FILE_B | Bitboard::FILE_C)
            - (Bitboard::RANK_1 | Bitboard::RANK_2);
        println!("{}\n{}", calced, expect);
        assert_eq!(calced, expect);

        let calced = PreCalc::instance().pawn_front_span_union_attack_span(Color::White, Square::A2);
        let expect = (Bitboard::FILE_A | Bitboard::FILE_B) - (Bitboard::RANK_1 | Bitboard::RANK_2);
        println!("{}\n{}", calced, expect);
        assert_eq!(calced, expect);

        let calced = PreCalc::instance().pawn_front_span_union_attack_span(Color::White, Square::H8);
        let expect = Bitboard::EMPTY;
        println!("{}\n{}", calced, expect);
        assert_eq!(calced, expect);

        let calced = PreCalc::instance().pawn_front_span_union_attack_span(Color::Black, Square::D7);
