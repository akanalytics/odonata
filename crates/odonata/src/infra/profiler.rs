...
use super::{
    black_box,
    utils::{DecimalFormatter, IntegerFormatter},
};
...
impl<'a> Flamegraph<'a> {
    ...
    pub fn report(&mut self) -> anyhow::Result<()> {
        ...
        report.flamegraph_with_options(file, &mut options)?;
        eprintln!(
            "Generated flamegraphs {name1} and {name2} with wd={wd}",
...
impl PerfProfiler {
    ...
    pub fn write<W: std::io::Write>(&mut self, mut w: W) -> anyhow::Result<()> {
        ...
        rt = (self.elapsed.as_secs_f64()).dp(5),
...
impl Drop for PerfProfiler {
    fn drop(&mut self) {
        if std::env::var("RUST_BENCH").is_err() {
            return;
        }
...
mod tests {
    ...
    use crate::{infra::black_box, test_log::test, trace::stat::Stat};
