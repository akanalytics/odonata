...
use crate::{infra::metric::Metrics, piece::Ply, prelude::*, search::node::Timing};
...
//
#[inline]
pub fn sigmoid_f64(centipawns: f64) -> f64 {
    1.0 / (1.0 + f64::exp(-centipawns))
}

pub fn file_open(filename: impl AsRef<Path>) -> anyhow::Result<File> {
    let f = File::open(&filename)
        .with_context(|| format!("failed to read file: {s}", s = filename.as_ref().display()))
        .with_context(|| {
            format!(
                "current dir {dir}",
                dir = std::env::current_dir().unwrap_or_default().display()
            )
        })?;
    Ok(f)
}
