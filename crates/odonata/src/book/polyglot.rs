...
use crate::{
    ...
    mv::{BareMove, Move},
    piece::{Hash, Ply},
    search::restrictions::Restrictions,
    variation::Variation,
    Color, Piece, infra::utils::file_open,
};
...
// COPYING
//
// A note on copyright (from the link above)
// I believe that the algorithm described below may be freely implemented by
// all GUIs, adapters and engines, including closed source ones.
// Polyglot itself is GPL but the GPL only covers actual code and not algorithms.
//
...
struct PolyglotParser<B: BufRead> {
    pub buf: B,
}

impl Debug for Polyglot {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.debug_struct("Polyglot")
            .field("entries", &self.entries.len())
            .finish()
    }
}

impl From<[u8; 16]> for PolyglotEntry {
...
impl PolyglotEntry {
    fn bare_move(&self) -> BareMove {
        ...
        let to_file = self.mv & 7;
        let to_rank = (self.mv >> 3) & 7;
        let to = Square::from_xy(to_file as u32, to_rank as u32);
        let from_file = (self.mv >> 6) & 7;
        let from_rank = (self.mv >> 9) & 7;
        let from = Square::from_xy(from_file as u32, from_rank as u32);
        let promo_num = (self.mv >> 12) & 7;
        let promo = match promo_num {
            0 => None,
            1 => Some(Piece::Knight),
            2 => Some(Piece::Bishop),
            3 => Some(Piece::Rook),
            4 => Some(Piece::Queen),
            x => panic!("Polyglot entry {self:?} has illegal promo field in mv of {x}"),
        };
        BareMove {
            mover: None,
            from,
    ...
    pub fn calc_move(&self, bd: &Board) -> Move {
        match self.validate_move(bd) {
            Ok(mv) => mv,
            Err(s) => panic!("error {s} calculating move for board {bd} from polyglot"),
        }
    }

    pub fn validate_move(&self, bd: &Board) -> anyhow::Result<Move> {
        let mut bm = self.bare_move();
        if bm.from.is_in(bd.kings()) && bm.promo.is_none() {
            let to = match (bm.from, bm.to) {
...
impl<B: BufRead> Iterator for PolyglotParser<B> {
    type Item = anyhow::Result<PolyglotEntry>;

    fn next(&mut self) -> Option<Self::Item> {
        const SIZE: usize = std::mem::size_of::<PolyglotEntry>();
        let mut buffer = [0_u8; SIZE];
        let res = self.buf.read(&mut buffer[..]);
        match res {
            Ok(0) => None,
            Ok(SIZE) => Some(Ok(PolyglotEntry::from(buffer))),
            Ok(_) => Some(Err(anyhow::anyhow!("Unexpected EOF"))),
            Err(e) => Some(Err(anyhow::anyhow!(e))),
        }
    }
}

impl<B: BufRead> PolyglotParser<B> {
    fn try_collect(self) -> anyhow::Result<Vec<PolyglotEntry>> {
...
impl Polyglot {
    ...
    pub fn load(&mut self, filename: &str) -> anyhow::Result<()> {
        ...
        self.entries.extend(vec.into_iter());
        self.entries.sort_by_key(|entry| entry.hash);
        Ok(())
    }

    pub fn find_all(&self) -> impl Iterator<Item = &PolyglotEntry> + '_ {
        self.entries.iter()
    }
    ...
    pub fn find_matching<'a>(
        ...
        // the elements are ordered so we can optimise better than a linear filter
        // such as .filter(move |entry| entry.hash == board_hash)
        let board_hash = self.hash_board(bd);
        let iter = self
            .entries
            .iter()
            .skip_while(move |entry| entry.hash != board_hash)
            .take_while(move |entry| entry.hash == board_hash)
            .filter(move |entry| !res.skip_move(0, entry.calc_move(bd)));
        iter
    }

    pub fn find_best_matching<'a>(
        &'a self,
        b: &'a Board,
        res: &'a Restrictions,
    ) -> impl Iterator<Item = &PolyglotEntry> + '_ {
    ...
    pub fn perft(&self, init: &Board, res: &Restrictions, d: Ply, f: &mut impl FnMut(&Variation)) {
        self.perft_var(init, res, &Variation::new(), d, f);
    }
    ...
    fn perft_var(
        ...
        var: &Variation,
        depth: Ply,
        f: &mut impl FnMut(&Variation),
    ) {
        f(var);
        if depth > 0 {
            let mut mvs = self.find_matching(b, restrictions).collect_vec();
            mvs.sort_by_key(|&ent| -(ent.weight as i32));
            for mv in mvs.iter().map(|e| e.calc_move(&b)) {
                let child = b.make_move(mv);
                self.perft_var(&child, restrictions, &var.append(mv), depth - 1, f);
            }
        }
    }
    ...
    pub fn hash_board(&self, b: &Board) -> Hash {
        ...
        for &p in &Piece::ALL {
            for sq in (b.pieces(p) & b.white()).squares() {
                let offset = Self::offset_piece(Color::White, p, sq);
                hash ^= RANDOM64[offset];
            }
            for sq in (b.pieces(p) & b.black()).squares() {
                let offset = Self::offset_piece(Color::Black, p, sq);
                hash ^= RANDOM64[offset];
            }
        }
        if b.castling().contains(CastlingRights::BLACK_KING) {
            hash ^= RANDOM64[Self::offset_castle(CastlingRights::BLACK_KING)];
        }
        if b.castling().contains(CastlingRights::WHITE_KING) {
            hash ^= RANDOM64[Self::offset_castle(CastlingRights::WHITE_KING)];
        }
        if b.castling().contains(CastlingRights::BLACK_QUEEN) {
            hash ^= RANDOM64[Self::offset_castle(CastlingRights::BLACK_QUEEN)];
        }
        if b.castling().contains(CastlingRights::WHITE_QUEEN) {
            hash ^= RANDOM64[Self::offset_castle(CastlingRights::WHITE_QUEEN)];
        }
        if let Some(offset) = Self::offset_en_passant(b) {
            hash ^= RANDOM64[offset];
        }

        if let Some(offset) = Self::offset_turn(b.color_us()) {
            hash ^= RANDOM64[offset];
        }
        hash
    }
    ...
    fn offset_piece(c: Color, p: Piece, sq: Square) -> usize {
        ...
        let offset_piece = 64 * kind_of_piece + 8 * sq.rank_index() + sq.file_index();
        offset_piece + 0
    }

    fn offset_castle(single_right: CastlingRights) -> usize {
        // white can castle short     0
        // white can castle long      1
        // black can castle short     2
        // black can castle long      3
        let offset_castle = match single_right {
            CastlingRights::WHITE_KING => 0,
            CastlingRights::WHITE_QUEEN => 1,
            CastlingRights::BLACK_KING => 2,
            CastlingRights::BLACK_QUEEN => 3,
            _ => unreachable!(),
        };
        offset_castle + 768
    }

    fn offset_en_passant(bd: &Board) -> Option<usize> {
        ...
        // file of the pushed pawn (counted from 0(=a) to 7(=h)).
        ...
        // "en passant target square" after a double pawn push is unconditional.
...
mod tests {
    ...
    fn test_polyglot_file() {
        ...
        let entries = polyglot.find_matching(&b, &res).collect_vec();
        assert_eq!(entries.len(), 8);

        let only_d4 = Restrictions {
            search_moves: vec![b.parse_san_move("d4").unwrap()].into(),
            ..Restrictions::default()
        };
        let ents = polyglot.find_matching(&b, &only_d4).collect_vec();
        assert_eq!(ents.len(), 1);
        assert_eq!(ents[0].calc_move(&b).to_san(&b), "d4");

        let not_c3 = Restrictions {
            excluded_moves: vec![b.parse_san_move("b1c3").unwrap()].into(),
            ..Restrictions::default()
        };
        let ents = polyglot.find_matching(&b, &not_c3).collect_vec();
        assert_eq!(ents.len(), 7);

        let mvs = entries
            .iter()
            .map(|e| (e.calc_move(&b).to_san(&b) + &format!(":{}", e.weight)));
        println!("Moves from {b} {}", mvs.format(", "));
        ...
        let mvs = entries
            .iter()
            .map(|e| (e.calc_move(&b).to_san(&b) + &format!(":{:2}", e.weight)));
        println!("Moves from {b} {}", mvs.format(", "));
    }
    ...
    fn test_polyglot_read() {
        ...
        assert_eq!(pge.mv, 0x0355); // 853
        assert_eq!(pge.weight, 1);
        assert_eq!(pge.learn, 0);

        let bytes = [bytes1, bytes2].concat();
        let mut cur = Cursor::new(&bytes);
        let parser = PolyglotParser { buf: &mut cur };
        let entries = parser.collect::<Result<Vec<_>, _>>();
        ...
        let bytes = [[bytes1, bytes2].concat(), vec![0x00]].concat();
        assert_eq!(bytes.len(), 33);
        let mut cur = Cursor::new(&bytes);
        let parser = PolyglotParser { buf: &mut cur };
        let entries = parser.try_collect();
        assert_eq!(entries.is_err(), true);
        assert_eq!(entries.err().unwrap().to_string(), "Unexpected EOF");
    }
    ...
    fn test_polyglot_perft() {
        ...
        polyglot.perft(&b, &res, 4, &mut |var| println!("var: {}", var.to_san(&b)));

        let mut count = 0;
        let mut max_ply = 0;
        polyglot.perft(&b, &res, 10, &mut |var| {
            count += 1;
            if var.len() > max_ply {
                max_ply = var.len();
                println!("{max_ply:2}: {}", var.to_san(&b));
            }
        });
    ...
    #[test]
    /// source of test cases: http://hgm.nubati.net/book_format.html
    fn test_polyglot_hash() {
        let hasher = Polyglot::default();

        // starting position
        let b = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "463b96181691fc9c");

        // position after e2e4
        let b = "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "823c9b50fd114196");

        // position after e2e4 d75
        let b = "rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 2".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "756b94461c50fb0");

        // position after e2e4 d7d5 e4e5
        let b = "rnbqkbnr/ppp1pppp/8/3pP3/8/8/PPPP1PPP/RNBQKBNR b KQkq - 0 2".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "662fafb965db29d4");

        // position after e2e4 d7d5 e4e5 f7f5
        let b = "rnbqkbnr/ppp1p1pp/8/3pPp2/8/8/PPPP1PPP/RNBQKBNR w KQkq f6 0 3".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "22a48b5a8e47ff78");

        // position after e2e4 d7d5 e4e5 f7f5 e1e2
        let b = "rnbqkbnr/ppp1p1pp/8/3pPp2/8/8/PPPPKPPP/RNBQ1BNR b kq - 0 3".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "652a607ca3f242c1");

        // position after e2e4 d7d5 e4e5 f7f5 e1e2 e8f7
        let b = "rnbq1bnr/ppp1pkpp/8/3pPp2/8/8/PPPPKPPP/RNBQ1BNR w - - 0 4".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "fdd303c946bdd9");

        // position after a2a4 b7b5 h2h4 b5b4 c2c4
        let b = "rnbqkbnr/p1pppppp/8/8/PpP4P/8/1P1PPPP1/RNBQKBNR b KQkq c3 0 3".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "3c8123ea7b067637");

        // position after a2a4 b7b5 h2h4 b5b4 c2c4 b4c3 a1a3
        let b = "rnbqkbnr/p1pppppp/8/8/P6P/R1p5/1P1PPPP1/1NBQKBNR b Kkq - 0 4".board();
        assert_eq!(format!("{:x}", hasher.hash_board(&b)), "5c3f9b829b279560");
    }
}
